...
int RStatBitCommand(RedisModuleCtx* ctx, RedisModuleString** argv, int argc) {
  ...
  RedisModule_ReplicateVerbatim(ctx);
  RedisModule_ReplyWithSimpleString(ctx, "Roaring");
  // sdsfree(s);
...
void *BitmapRdbLoad(RedisModuleIO *rdb, int encver) {
  if (encver != BITMAP_ENCODING_VERSION) {
    RedisModule_LogIOError(rdb, "warning", "Can't load data with version %d",
                           encver);
    return NULL;
  }
  size_t size;
...
void BitmapAofRewrite(RedisModuleIO *aof, RedisModuleString *key, void *value) {
  Bitmap *bitmap = value;
  Bitmap_aof_rewrite_callback_params params = {.aof = aof, .key = key};
...
void BitmapFree(void *value) { bitmap_free(value); }
